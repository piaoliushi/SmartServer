...
namespace net
{
	dev_upload_handler_X::dev_upload_handler_X(boost::asio::io_service& io_service, 
...
#endif 
		,taskwork_(taskwork)
		,receive_msg_ptr_(new dev_upload_message(1048,dev_agent_))
		,srv_(srv)
		...
		,task_count_(0)
	{
		cur_upload_dev_info_.nConnectType=-1;
	}

	dev_upload_handler_X::~dev_upload_handler_X(void)
	{

	}

	//关闭连接，并清理自己
	void dev_upload_handler_X::close_handler()
	{
		h_b_timer_.cancel();
		close_i();
	...
	//关闭
// 	void dev_upload_handler_X::close_i()
// 	{
// 		
...
// 	}
	...
	//启动心跳定时器
	void dev_upload_handler_X::start_hb_timer()
	{

		h_b_timer_.expires_from_now(boost::posix_time::seconds(60));
		h_b_timer_.async_wait(strand_.wrap(
			boost::bind(&dev_upload_handler_X::handle_timeout,this,
			boost::asio::placeholders::error))); 
	}

	//心跳超时
	void dev_upload_handler_X::handle_timeout(const boost::system::error_code& e)
	{
		if (!e)
		{
	...
	//获得最后心跳时间
	std::time_t dev_upload_handler_X::get_last_hb_time()
	{
	...
	//设置最后心跳时间
	void dev_upload_handler_X::set_last_hb_time(std::time_t t)
	{
		boost::mutex::scoped_lock lock(hb_mutex_);
		last_hb_time_ = t;
	}
	...
	//回复心跳请求
	int dev_upload_handler_X::heart_beat_rsp()
	{
		std::time_t now = time(0);
	...
	//提交任务
	/*void dev_upload_handler_X::task_count_increase()
	{
		boost::mutex::scoped_lock lock(task_mutex_);
		++task_count_;
	}
	//任务递减
	void dev_upload_handler_X::task_count_decrease()
	{
		boost::mutex::scoped_lock lock(task_mutex_);
		--task_count_;
	...
	int dev_upload_handler_X::task_count()
	{
		boost::mutex::scoped_lock lock(task_mutex_);
		return task_count_;
	}
	...
	//等待任务结束
	void dev_upload_handler_X::wait_task_end()
	{
		boost::mutex::scoped_lock lock(task_mutex_);
		while(task_count_>0)
		{
	...
	//判断当前时间是否需要保存监控数据
	bool dev_upload_handler_X::is_need_save_data()
	{
		time_t tmCurTime;
		time(&tmCurTime);
	...
	//开始接收消息头
	void dev_upload_handler_X::start_read_head()
	{
	...
	//消息头接收回调
	void dev_upload_handler_X::handle_read_head(const boost::system::error_code& error,
	...
	void dev_upload_handler_X::handle_read_body(const boost::system::error_code& error, 
		                                      size_t bytes_transferred)
	{
		//if(srv_.isExitServer())
		//	return;
...
// 				if(receive_msg_ptr_->decode_msg(curData_ptr,bytes_transferred)>=0)
// 				{
// 					if(boost::detail::thread::singleton<boost::threadpool::pool>::instance()
// 						.schedule(boost::bind(&dev_upload_handler_X::handler_data,this,curData_ptr)))
// 					{
// 						task_count_increase();
// 					}
// 				}
...
// 				curAudioData->assign(receive_msg_ptr_->get_msg().begin()+UPLOAD_MSG_HEAD_SIZE+1
// 					                 ,receive_msg_ptr_->get_msg().end()-1);
// 				boost::uint8_t uChannel = *(receive_msg_ptr_->body());
// 				if(boost::detail::thread::singleton<boost::threadpool::pool>::instance()
// 					.schedule(boost::bind(&dev_upload_handler_X::handler_mp3_data,this,uChannel,curAudioData)))
// 				{
// 					task_count_increase();
// 				}
	...
	void dev_upload_handler_X::handler_mp3_data(boost::uint8_t uChannel,unchar_ptr curDataPtr)
	{
	...
	//监测报警,保存数据
	void dev_upload_handler_X::handler_data(DevMonitorDataPtr curDataPtr)
	{
		...
		bool bIsMonitorTime = is_monitor_time();
		if(bIsMonitorTime)
		...
		if(bIsMonitorTime)
			...
			std::cout<<"sorry，监测时间段以外"<<endl;

		task_count_decrease();*/
	}
	
	//检测报警状态
/*	void dev_upload_handler_X::detect_alarm_state(DevMonitorDataPtr curDataPtr,bool bMonitor)
	{
	...
	//当前项是否在报警
	bool dev_upload_handler_X::ItemValueIsAlarm(float fValue,DevParamerMonitorItem &ItemInfo,
	...
	//是否在监测时间段
	bool dev_upload_handler_X::is_monitor_time()
	{
		time_t curTime = time(0);
	...
	//判断是否保存当前记录
	void dev_upload_handler_X::save_monitor_record(DevMonitorDataPtr curDataPtr)
	{
		if(is_need_save_data())
		{

			GetInst(DataBaseManager).SaveOthDevMonitoringData(cur_upload_dev_info_.sStationNumber,
							cur_upload_dev_info_.sDevNum,cur_upload_dev_info_.mapMonitorItem,curDataPtr);
		}

	}*/

	//清除所有报警标志
	void dev_upload_handler_X::clear_all_alarm()
	{
		/*boost::recursive_mutex::scoped_lock lock(alarm_state_mutex);
	...
	//解析设备回复命令
	bool dev_upload_handler_X::handler_command_data(boost::uint8_t cmdType,uploadMsgPtr pMsg)
	{
		...
		case 0xEE://登陆
			{
				...
				if(GetInst(StationConfig).get_upload_dev_info(msg_->devAddress,cur_upload_dev_info_))
				{
					...
					dev_agent_.HxInit(cur_upload_dev_info_.nDevProtocol, 
									  cur_upload_dev_info_.nSubProtocol,
									  cur_upload_dev_info_.nDevAddr,
						              itemRadio);
	...
	/*void dev_upload_handler_X::set_con_state(con_state curState)
	{
	...
	void dev_upload_handler_X::set_run_state(dev_run_state curState)
	{
	...
	con_state  dev_upload_handler_X::get_con_state()
	{
		boost::recursive_mutex::scoped_lock llock(con_state_mutex_);
		return dev_con_state_;
	}

	dev_run_state dev_upload_handler_X::get_run_state()
	{
		boost::recursive_mutex::scoped_lock llock(run_state_mutex_);
		return dev_run_state_;
	}

	void dev_upload_handler_X::get_alarm_state(map<int,std::pair<int,tm>>& cellAlarm)
	{

	}
	
	void dev_upload_handler_X::dev_base_info(DevBaseInfo& devInfo,string iId)
	{
		devInfo.mapMonitorItem = cur_upload_dev_info_.mapMonitorItem;
		devInfo.nDevType = DEV_OTHER;
		...
		devInfo.nConnectType = 3;
	}*/

	void dev_upload_handler_X::start_write(msgPointer msgPtr)
	{
	...
	//切换音频通道
	bool dev_upload_handler_X::switch_audio_channel(string sUser,int sNewChannel,e_ErrorCode &opResult)
	{
		/*if(get_con_state()!=con_connected)
			return false;
	...
	//执行通用指令
	bool dev_upload_handler_X::excute_command(int cmdType,devCommdMsgPtr lpParam,e_ErrorCode &opResult)
	{
		/*if(get_con_state()!=con_connected)
			return false;
	...
	//发送google消息
	bool dev_upload_handler_X::sendMessage(e_MsgType _type,googleMsgPtr gMsgPtr)
	{
		msgPointer msgPtr(new message);
	...
	void dev_upload_handler_X::handle_write(const boost::system::error_code& error,size_t bytes_transferred)
	{
		if (!error)
		{
		}
