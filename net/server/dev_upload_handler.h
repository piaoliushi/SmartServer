#ifndef DEV_UPLOAD_HANDLER
#define DEV_UPLOAD_HANDLER
...
namespace net
{
	class dev_upload_handler:public session
	{
	public:	
		dev_upload_handler(boost::asio::io_service& io_service, 
			TaskQueue<msgPointer>& taskwork,server& srv);

		virtual ~dev_upload_handler(void);
		//开始接收消息头
	...
	protected:
		...
		void wait_task_end();
		//清除通道告警
		void clear_channel_alarm(int nChannelId);
		//清除所有报警标志
		void clear_all_alarm();
		//是否在监测时间段
		bool is_monitor_time(int nChannel);
		//是否是录制时间段
		bool is_record_time(int nChannel);
		//判断是否保存当前记录
		void save_monitor_record(int nChannelId,DevMonitorDataPtr curDataPtr);
		//检测报警状态
		void detect_alarm_state(int nChannelId,DevMonitorDataPtr curDataPtr,bool bMonitor);
		//当前项是否在报警
	...
	private:
		...
		HMsgHandlePtr                   pars_agent_;
		uploadMsgPtr                    receive_msg_ptr_;//接收缓冲区
		MediaDeviceParamInfo            cur_upload_dev_info_;//当前上传设备信息
