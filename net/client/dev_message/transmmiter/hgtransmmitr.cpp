...
int HgTransmmitr::decode_msg_body(unsigned char *data, DevMonitorDataPtr data_ptr, int nDataLen, int &runstate)
{
   ...
   case HAERBIN_HG4_1kw:
       return Hg1Kw4Data(data,data_ptr,nDataLen,runstate);
   }
   return RE_NOPROTOCOL;
}
...
int HgTransmmitr::Hg1Kw4Data(unsigned char *data, DevMonitorDataPtr data_ptr, int nDataLen, int &runstate)
{
    char ResultData[10]={0};
    int HiByte=-1,Bmodel=-1;
    DataInfo dtinfo;
    ...
    if((data_ptr->mValues[0].fValue-data_ptr->mValues[1].fValue)<=0)
    {
        data_ptr->mValues[2].fValue = 0;
    }
    else
    {
        data_ptr->mValues[2].fValue = (data_ptr->mValues[0].fValue+data_ptr->mValues[1].fValue)/(data_ptr->mValues[0].fValue-data_ptr->mValues[1].fValue);
    }
    return RE_SUCCESS;
}
}
