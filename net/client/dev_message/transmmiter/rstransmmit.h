#ifndef RSTRANSMMIT_H
#define RSTRANSMMIT_H
#pragma once
#include "Transmmiter.h"
#include "../Tsmt_message.h"
namespace hx_net{
class RsTransmmit:public Transmmiter
{
public:
    RsTransmmit(Tsmt_message_ptr ptsmessage,int subprotocol,int addresscode);
    ...
    void rs_Callback(int reason, Snmp *session,Pdu &pdu, SnmpTarget &target);
  //  bool exec_cmd(Snmp *snmp, int cmdtype, CTarget *target);
protected:
    void initOid();
private:
    int get_snmp(Snmp *snmp,DevMonitorDataPtr data_ptr,CTarget *target);
    int GermanyRSData(unsigned char *data,DevMonitorDataPtr data_ptr,int nDataLen,int& runstate);
    void rs_decode(int reason, Snmp *session,Pdu &pdu, SnmpTarget &target);
private:
    int m_subprotocol;
    int m_addresscode;
    Pdu query_pdu;
