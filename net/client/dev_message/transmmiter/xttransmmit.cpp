...
int XtTransmmit::decode_msg_body(unsigned char *data, DevMonitorDataPtr data_ptr, int nDataLen, int &runstate)
{
    ...
    case XT_DM_1KW:
        return xtDM_1KW_Data(data,data_ptr,nDataLen,runstate);
    }
    return RE_NOPROTOCOL;
}
...
int XtTransmmit::xtDM_1KW_Data(unsigned char *data, DevMonitorDataPtr data_ptr, int nDataLen, int &runstate)
{
    if(data[4]!=0x03)
            return RE_CMDACK;
    DataInfo dtinfo;
    ...
    dtinfo.fValue = (float)(data[13]/255.000000);
    data_ptr->mValues[0] = dtinfo;
    dtinfo.fValue = (float)(data[12]/255.000000);
    data_ptr->mValues[1] = dtinfo;

    dtinfo.fValue = 0;
    data_ptr->mValues[2] = dtinfo;

    dtinfo.fValue = (float)(data[8]/255.000000);
    data_ptr->mValues[3] = dtinfo;


    dtinfo.fValue = (float)(data[10]/255.000000);
    data_ptr->mValues[4] = dtinfo;


    dtinfo.fValue = (float)(data[11]/255.000000);
    data_ptr->mValues[5] = dtinfo;
