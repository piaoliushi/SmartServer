 ...
 *
 * 当程序结束后，需要调用 ConnectionPool::release() 关闭所有数据库的连接（一般在 main() 函数返回前调用）。
 *
 * 使用方法：
 * 1. 从数据库连接池里取得连接
...
public:
    static void initDb(const QString& serveraddress, const QString& database, const QString& uid, const QString& pwd);
